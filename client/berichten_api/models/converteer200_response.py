...
class Converteer200Response(BaseModel):
    """
    Converteer200Response
    """ # noqa: E501
    bericht_inhoud: Optional[LoBericht] = Field(default=None, alias="berichtInhoud")
    validatie_fouten: Optional[List[Converteer200ResponseValidatieFoutenInner]] = Field(default=None, alias="validatieFouten")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["berichtInhoud", "validatieFouten"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of bericht_inhoud
        if self.bericht_inhoud:
            _dict['berichtInhoud'] = self.bericht_inhoud.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in validatie_fouten (list)
        _items = []
        if self.validatie_fouten:
            for _item_validatie_fouten in self.validatie_fouten:
                if _item_validatie_fouten:
                    _items.append(_item_validatie_fouten.to_dict())
            _dict['validatieFouten'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "berichtInhoud": LoBericht.from_dict(obj["berichtInhoud"]) if obj.get("berichtInhoud") is not None else None,
            "validatieFouten": [Converteer200ResponseValidatieFoutenInner.from_dict(_item) for _item in obj["validatieFouten"]] if obj.get("validatieFouten") is not None else None
        })
