...
CONVERTEER406RESPONSE_ONE_OF_SCHEMAS = ["BBACONVF001", "BBACONVF005"]

class Converteer406Response(BaseModel):
    """
    ...
    oneof_schema_2_validator: Optional[BBACONVF005] = None
    actual_instance: Optional[Union[BBACONVF001, BBACONVF005]] = None
    one_of_schemas: Set[str] = { "BBACONVF001", "BBACONVF005" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = Converteer406Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in Converteer406Response with oneOf schemas: BBACONVF001, BBACONVF005. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in Converteer406Response with oneOf schemas: BBACONVF001, BBACONVF005. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-CONV-F001":
            instance.actual_instance = BBACONVF001.from_json(json_str)
            return instance

        # check if data type is `BBACONVF005`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-CONV-F005":
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into Converteer406Response with oneOf schemas: BBACONVF001, BBACONVF005. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Converteer406Response with oneOf schemas: BBACONVF001, BBACONVF005. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BBACONVF001, BBACONVF005]]:
