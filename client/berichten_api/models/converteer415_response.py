...
CONVERTEER415RESPONSE_ONE_OF_SCHEMAS = ["BBACONVF002", "BBACONVF003"]

class Converteer415Response(BaseModel):
    """
    Converteer415Response
    """
    # data type: BBACONVF002
    oneof_schema_1_validator: Optional[BBACONVF002] = None
    # data type: BBACONVF003
    oneof_schema_2_validator: Optional[BBACONVF003] = None
    actual_instance: Optional[Union[BBACONVF002, BBACONVF003]] = None
    one_of_schemas: Set[str] = { "BBACONVF002", "BBACONVF003" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = Converteer415Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in Converteer415Response with oneOf schemas: BBACONVF002, BBACONVF003. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in Converteer415Response with oneOf schemas: BBACONVF002, BBACONVF003. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into Converteer415Response with oneOf schemas: BBACONVF002, BBACONVF003. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Converteer415Response with oneOf schemas: BBACONVF002, BBACONVF003. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BBACONVF002, BBACONVF003]]:
