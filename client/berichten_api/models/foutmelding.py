...
class Foutmelding(BaseModel):
    """
    Foutmelding
    """ # noqa: E501
    type: StrictStr = Field(description="Unieke (RFC7807) foutcode voor het type van de opgetreden fout. Een namespace URI-pad gevolgd door een type fout id in de vorm VVV-DDDD-FFFF waarbij VVV gelijk is aan de afkorting van de voorziening (in dit geval BBA voor BRP Berichten API). DDDD een indicatie geeft van de operatie/domein waar de fout is opgetreden en FFFF een opdeling betreft van specifieke fouten (types) welke binnen een domein zijn identificeert. ")
    title: StrictStr = Field(description="De titel (RFC7807) geeft een korte uitleg van het soort fout en in sommige gevallen een toelichting van de oorzaak. ")
    detail: Optional[StrictStr] = Field(default=None, description="Dit (RFC7807) veld geeft toelichting die kan helpen om de oorzaak van de fout te achterhalen, of geeft achtergrondinformatie over de foutsituatie. ")
    status: Optional[StrictInt] = Field(default=None, description="Dit (RFC7807) veld beschrijft de HTTP status-code die door de BRP-Berichten-API is toegekend aan de fout (indien van toepassing).")
    instance: Optional[StrictStr] = Field(default=None, description="Dit (RFC7807) veld beschrijft het request waarbij dit probleem is opgetreden (indien van toepassing).")
    invalid_parameters: Optional[List[InvalidParametersInner]] = Field(default=None, alias="invalidParameters")
    date_time: Optional[datetime] = Field(default=None, description="Het tijdstip en de datum (ISO date/time) waarop de fout is afgevangen en het foutmelding object is samengesteld. ", alias="dateTime")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "title", "detail", "status", "instance", "invalidParameters", "dateTime"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.invalid_parameters:
            ...
            _dict['invalidParameters'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "invalidParameters": [InvalidParametersInner.from_dict(_item) for _item in obj["invalidParameters"]] if obj.get("invalidParameters") is not None else None,
