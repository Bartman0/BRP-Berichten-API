...
class GetMessageAntwoord(BaseModel):
    """
    GetMessageAntwoord
    """ # noqa: E501
    interactie_id: StrictStr = Field(description="Een uniek ID wat aan het HTTP request gekoppeld wordt.", alias="interactieId")
    opgehaalde_berichten: List[GetMessageAntwoordAllOfOpgehaaldeBerichten] = Field(description="Gevraagde berichten die ook succesvol opgehaald konden worden.", alias="opgehaaldeBerichten")
    niet_opgehaalde_berichten: List[GetMessageAntwoordAllOfNietOpgehaaldeBerichten] = Field(description="Gevraagde berichten die niet succesvol opgehaald konden worden.", alias="nietOpgehaaldeBerichten")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["interactieId", "opgehaaldeBerichten", "nietOpgehaaldeBerichten"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in opgehaalde_berichten (list)
        _items = []
        if self.opgehaalde_berichten:
            for _item_opgehaalde_berichten in self.opgehaalde_berichten:
                if _item_opgehaalde_berichten:
                    _items.append(_item_opgehaalde_berichten.to_dict())
            _dict['opgehaaldeBerichten'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in niet_opgehaalde_berichten (list)
        _items = []
        if self.niet_opgehaalde_berichten:
            for _item_niet_opgehaalde_berichten in self.niet_opgehaalde_berichten:
                if _item_niet_opgehaalde_berichten:
                    _items.append(_item_niet_opgehaalde_berichten.to_dict())
            _dict['nietOpgehaaldeBerichten'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "interactieId": obj.get("interactieId"),
            "opgehaaldeBerichten": [GetMessageAntwoordAllOfOpgehaaldeBerichten.from_dict(_item) for _item in obj["opgehaaldeBerichten"]] if obj.get("opgehaaldeBerichten") is not None else None,
            "nietOpgehaaldeBerichten": [GetMessageAntwoordAllOfNietOpgehaaldeBerichten.from_dict(_item) for _item in obj["nietOpgehaaldeBerichten"]] if obj.get("nietOpgehaaldeBerichten") is not None else None
        })
