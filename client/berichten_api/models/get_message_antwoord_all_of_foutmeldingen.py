...
GETMESSAGEANTWOORDALLOFFOUTMELDINGEN_ONE_OF_SCHEMAS = ["BBAGETF001", "BBAGETF002", "BBAGETF003"]

class GetMessageAntwoordAllOfFoutmeldingen(BaseModel):
    """
    GetMessageAntwoordAllOfFoutmeldingen
    """
    # data type: BBAGETF001
    oneof_schema_1_validator: Optional[BBAGETF001] = None
    # data type: BBAGETF002
    oneof_schema_2_validator: Optional[BBAGETF002] = None
    # data type: BBAGETF003
    oneof_schema_3_validator: Optional[BBAGETF003] = None
    actual_instance: Optional[Union[BBAGETF001, BBAGETF002, BBAGETF003]] = None
    one_of_schemas: Set[str] = { "BBAGETF001", "BBAGETF002", "BBAGETF003" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GetMessageAntwoordAllOfFoutmeldingen.model_construct()
        error_messages = []
        match = 0
        # validate data type: BBAGETF001
        if not isinstance(v, BBAGETF001):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BBAGETF001`")
        else:
            match += 1
        # validate data type: BBAGETF002
        if not isinstance(v, BBAGETF002):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BBAGETF002`")
        else:
            match += 1
        # validate data type: BBAGETF003
        if not isinstance(v, BBAGETF003):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BBAGETF003`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GetMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAGETF001, BBAGETF002, BBAGETF003. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GetMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAGETF001, BBAGETF002, BBAGETF003. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # check if data type is `BBAGETF001`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-GET-F001":
            instance.actual_instance = BBAGETF001.from_json(json_str)
            return instance

        # check if data type is `BBAGETF002`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-GET-F002":
            instance.actual_instance = BBAGETF002.from_json(json_str)
            return instance

        # check if data type is `BBAGETF003`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-GET-F003":
            instance.actual_instance = BBAGETF003.from_json(json_str)
            return instance

        # check if data type is `BBAGETF001`
        if _data_type == "BBA-GET-F001":
            instance.actual_instance = BBAGETF001.from_json(json_str)
            return instance

        # check if data type is `BBAGETF002`
        if _data_type == "BBA-GET-F002":
            instance.actual_instance = BBAGETF002.from_json(json_str)
            return instance

        # check if data type is `BBAGETF003`
        if _data_type == "BBA-GET-F003":
            instance.actual_instance = BBAGETF003.from_json(json_str)
            return instance

        # deserialize data into BBAGETF001
        try:
            instance.actual_instance = BBAGETF001.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BBAGETF002
        try:
            instance.actual_instance = BBAGETF002.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BBAGETF003
        try:
            instance.actual_instance = BBAGETF003.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GetMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAGETF001, BBAGETF002, BBAGETF003. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAGETF001, BBAGETF002, BBAGETF003. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BBAGETF001, BBAGETF002, BBAGETF003]]:
