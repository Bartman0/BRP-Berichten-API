...
class ListMessageAntwoord(BaseModel):
    """
    ListMessageAntwoord
    """ # noqa: E501
    interactie_id: StrictStr = Field(description="Een uniek ID wat aan het HTTP request gekoppeld wordt.", alias="interactieId")
    paginering: PagineringResultaat
    berichten: List[ListMessageKenmerken]
    __properties: ClassVar[List[str]] = ["interactieId", "paginering", "berichten"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of paginering
        if self.paginering:
            _dict['paginering'] = self.paginering.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in berichten (list)
        _items = []
        if self.berichten:
            for _item_berichten in self.berichten:
                if _item_berichten:
                    _items.append(_item_berichten.to_dict())
            _dict['berichten'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "paginering": PagineringResultaat.from_dict(obj["paginering"]) if obj.get("paginering") is not None else None,
            "berichten": [ListMessageKenmerken.from_dict(_item) for _item in obj["berichten"]] if obj.get("berichten") is not None else None
        })
        return _obj
