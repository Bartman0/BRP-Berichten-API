...
class PutMessageAntwoord(BaseModel):
    """
    PutMessageAntwoord
    """ # noqa: E501
    interactie_id: StrictStr = Field(description="Een uniek ID wat aan het HTTP request gekoppeld wordt.", alias="interactieId")
    verwerkte_berichten: List[PutMessageAntwoordAllOfVerwerkteBerichten] = Field(alias="verwerkteBerichten")
    niet_verwerkte_berichten: List[PutMessageAntwoordAllOfNietVerwerkteBerichten] = Field(alias="nietVerwerkteBerichten")
    __properties: ClassVar[List[str]] = ["interactieId", "verwerkteBerichten", "nietVerwerkteBerichten"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in verwerkte_berichten (list)
        _items = []
        if self.verwerkte_berichten:
            for _item_verwerkte_berichten in self.verwerkte_berichten:
                if _item_verwerkte_berichten:
                    _items.append(_item_verwerkte_berichten.to_dict())
            _dict['verwerkteBerichten'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in niet_verwerkte_berichten (list)
        _items = []
        if self.niet_verwerkte_berichten:
            for _item_niet_verwerkte_berichten in self.niet_verwerkte_berichten:
                if _item_niet_verwerkte_berichten:
                    _items.append(_item_niet_verwerkte_berichten.to_dict())
            _dict['nietVerwerkteBerichten'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "verwerkteBerichten": [PutMessageAntwoordAllOfVerwerkteBerichten.from_dict(_item) for _item in obj["verwerkteBerichten"]] if obj.get("verwerkteBerichten") is not None else None,
            "nietVerwerkteBerichten": [PutMessageAntwoordAllOfNietVerwerkteBerichten.from_dict(_item) for _item in obj["nietVerwerkteBerichten"]] if obj.get("nietVerwerkteBerichten") is not None else None
        })
        return _obj
