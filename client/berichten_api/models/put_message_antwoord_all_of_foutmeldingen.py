...
PUTMESSAGEANTWOORDALLOFFOUTMELDINGEN_ONE_OF_SCHEMAS = ["BBAPUTF002", "BBAPUTF003", "BBAPUTF004"]

class PutMessageAntwoordAllOfFoutmeldingen(BaseModel):
    """
    PutMessageAntwoordAllOfFoutmeldingen
    """
    # data type: BBAPUTF002
    oneof_schema_1_validator: Optional[BBAPUTF002] = None
    # data type: BBAPUTF003
    oneof_schema_2_validator: Optional[BBAPUTF003] = None
    # data type: BBAPUTF004
    oneof_schema_3_validator: Optional[BBAPUTF004] = None
    actual_instance: Optional[Union[BBAPUTF002, BBAPUTF003, BBAPUTF004]] = None
    one_of_schemas: Set[str] = { "BBAPUTF002", "BBAPUTF003", "BBAPUTF004" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PutMessageAntwoordAllOfFoutmeldingen.model_construct()
        error_messages = []
        match = 0
        # validate data type: BBAPUTF002
        if not isinstance(v, BBAPUTF002):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BBAPUTF002`")
        else:
            match += 1
        # validate data type: BBAPUTF003
        if not isinstance(v, BBAPUTF003):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BBAPUTF003`")
        else:
            match += 1
        # validate data type: BBAPUTF004
        if not isinstance(v, BBAPUTF004):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BBAPUTF004`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PutMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAPUTF002, BBAPUTF003, BBAPUTF004. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PutMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAPUTF002, BBAPUTF003, BBAPUTF004. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # check if data type is `BBAPUTF002`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-PUT-F002":
            instance.actual_instance = BBAPUTF002.from_json(json_str)
            return instance

        # check if data type is `BBAPUTF003`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-PUT-F003":
            instance.actual_instance = BBAPUTF003.from_json(json_str)
            return instance

        # check if data type is `BBAPUTF004`
        if _data_type == "https://www.rvig.nl/brp/berichten-api/probleem/BBA-PUT-F004":
            instance.actual_instance = BBAPUTF004.from_json(json_str)
            return instance

        # check if data type is `BBAPUTF002`
        if _data_type == "BBA-PUT-F002":
            instance.actual_instance = BBAPUTF002.from_json(json_str)
            return instance

        # check if data type is `BBAPUTF003`
        if _data_type == "BBA-PUT-F003":
            instance.actual_instance = BBAPUTF003.from_json(json_str)
            return instance

        # check if data type is `BBAPUTF004`
        if _data_type == "BBA-PUT-F004":
            instance.actual_instance = BBAPUTF004.from_json(json_str)
            return instance

        # deserialize data into BBAPUTF002
        try:
            instance.actual_instance = BBAPUTF002.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BBAPUTF003
        try:
            instance.actual_instance = BBAPUTF003.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into BBAPUTF004
        try:
            instance.actual_instance = BBAPUTF004.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PutMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAPUTF002, BBAPUTF003, BBAPUTF004. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PutMessageAntwoordAllOfFoutmeldingen with oneOf schemas: BBAPUTF002, BBAPUTF003, BBAPUTF004. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BBAPUTF002, BBAPUTF003, BBAPUTF004]]:
