...
class Xa01(BaseModel):
    ...
    pl_data_set: List[Af01PlData] = Field(alias="plDataSet")
    __properties: ClassVar[List[str]] = ["$schema", "berichtType", "plDataSet"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in pl_data_set (list)
        _items = []
        if self.pl_data_set:
            for _item_pl_data_set in self.pl_data_set:
                if _item_pl_data_set:
                    _items.append(_item_pl_data_set.to_dict())
            _dict['plDataSet'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "plDataSet": [Af01PlData.from_dict(_item) for _item in obj["plDataSet"]] if obj.get("plDataSet") is not None else None
        })
        return _obj
